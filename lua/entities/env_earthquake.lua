...
function ENT:PlayInitialSounds()
	if self.Magnitude > 5 then
		CreateSoundWave("disasters/nature/earthquake_strong.mp3", self:GetPos(), "stereo" ,340.29/2, {90,110}, 5)
	else
		CreateSoundWave("disasters/nature/earthquake_weak.mp3", self:GetPos(), "stereo" ,340.29/2, {90,110}, 5)
	end
...
function ENT:CreateEarthquakeWithParent()
	...
	local decider = math.random(1,math.floor(self.Magnitude * 2))==1 -- earthquakes with lower magnitude have higher chance of spawning a foreshock!
	...
	if decider == false then
		...
		if  math.floor(self.Magnitude) > 1 then -- don't want magnitude 1 earthquakes to spawn more... do we? ye we don't so stop reading this comment before I contact FBI
			CreateSoundWave("disasters/nature/earthquake_aftershock.mp3", self:GetPos(), "stereo" ,340.29/2, {40,100}, 10)
		...
		CreateSoundWave("disasters/nature/earthquake_aftershock.mp3", self:GetPos(), "stereo" ,340.29/2, {120,130}, 20)
		CreateSoundWave("disasters/nature/earthquake_aftershock.mp3", self:GetPos(), "stereo" ,340.29/2.1, {120,130}, 20)
		

		local foreshock_magnitude = math.Clamp( math.floor(self.Magnitude) + math.random(3,6), 1, 10)
		...
		ParticleEffect("earthquake_swave_main", self.Parent:GetPos() + Vector(0,0,10), Angle(0,0,0), nil)
			...
			-- FORESHOCK (YOU'RE COMPLETELY FUCKED!)
	end
...
function ENT:EarthquakeDecay()
	...
	if math.random(1,2)==1 then -- to aftershock or not to aftershock
...
function ENT:SendClientsideEffects( ply, offset_ang, amplitude )
    ...
    if magnitude >= 6 then
		...
		if math.random(1, 21 - magnitude)==1 then
			net.Start("gd_clParticles")
			net.WriteString("earthquake_player_ground_debris", Angle(0,math.random(1,40),0))
			net.Send(ply)
		end	
	...
	for k, v in pairs(ents.GetAll()) do
		...
		if phys:IsValid()  and  (v:GetClass()!= "phys_constraintsystem" and v:GetClass()!= "phys_constraint"  and v:GetClass()!= "logic_collision_pair") then
			...
			if mag >= 3 and mag <=3.9 then
				...
				if mass < 60 then -- magnitude 3-4.9 earthquakes shouldn't have an effect on props of large mass
			...
			elseif mag >= 9 and mag <= 9.9 then
				...
				if mass < 37200 then 
					...
					if math.random(1, 2) == 1 then
						...
						phys:AddAngleVelocity( ang_v * 16 )
						phys:AddVelocity( ang_v * 8)
						self:Unfreeze(phys, v, mag)
			...
			elseif mag >= 10 and mag <= 11 then
				...
				if mass <= 50000 then 
					...
					if math.random(1, 2) == 1 then
						phys:AddAngleVelocity( ang_v * 32 )
						phys:AddVelocity( ang_v * 16)
						self:Unfreeze(phys, v, mag)
					end
...
function ENT:MagnitudeModulateSound()
	...
	if LocalPlayer().Sounds["Earthquake"]==nil then
		...
		LocalPlayer().Sounds["Earthquake"] = {  ["sound"] = createLoopedSound(LocalPlayer(), "disasters/nature/earthquake.wav"), ["volume"] = 0 }

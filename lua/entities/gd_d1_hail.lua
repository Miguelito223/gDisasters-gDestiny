...
function ENT:PhysicsCollide( data, phys )
	...
	if ( data.Speed > 500 ) then 
		...
		if self:GetModel()=="models/ramses/models/nature/hail_01.mdl" then
			...
			sound.Play(table.Random({"streams/event/break/ice_break_a.mp3","streams/event/break/ice_break_b.mp3","streams/event/break/ice_break_c.mp3"}), self:GetPos(), 80, math.Clamp(80,120), 1)

...
function ENT:Initialize()		
	...
	if (SERVER) then
		...
		GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=math.random(35,45),["Direction"]=Vector(1,0,0)}, ["Pressure"]    = 97000, ["Temperature"] = math.random(-10,-5), ["Humidity"]    = math.random(20,45), ["BRadiation"]  = 0.1}}
...
function ENT:AffectPlayers()
	for k, v in pairs(player.GetAll()) do
		...
		if v.gDisasters.Area.IsOutdoor then
			...
			net.WriteString("localized_heavy_snow_effect", Angle(0,math.random(1,40),0))
			net.Send(v)	

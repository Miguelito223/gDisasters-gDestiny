if (CLIENT) then
	...
	function gfx_temperatureEffects()
		...
		local vomit_intensity = LocalPlayer().gDisasters.HUD.VomitIntensity
		...
		local cm_hot = {}
			...
			cm_hot[ "$pp_colour_addg" ]        = 0 + vomit_intensity
...
function gDisasters_ProcessTemperature()
	...
	local function updateVars()
		for k, v in pairs(plytbl) do
			...
			local closest_fire, distance  = FindNearestEntity(v, "entityflame") -- find closest fire entity
			...
			if closest_fire != nil then
				heatscale = math.Clamp(1000/distance^2, 0,1) -- inverse square law
			end
				...
				timer.Create( "Breathing", 4, 0, function()
					for k, v in pairs(player.GetAll()) do
						timer.Simple(math.random(0,20)/10, function()
							...
							v:EmitSound("disasters/player/exhale.wav", 80, 100, 0.2)
						end)
					end

if !CLIENT then return end 

--[[
Ammo
--]]


--[[ 
Buildings
 --]]

 --[[
 Building 01
 --]]
 
AddToGDSpawnMenu("Static House 1"             , "gd_building_h1"           , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Large Roof Beam"             , "gd_building_h1_p01"       , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Single Stabilized Roof Support Beam", "gd_building_h1_p02" , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Long Metallic Roof Support Beam", "gd_building_h1_p03" , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Short Metallic Roof Support Beam", "gd_building_h1_p04" , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Aluminium Roof Tile", "gd_building_h1_p05" , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Double Stabilized Roof Support Beam", "gd_building_h1_p06" , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Aluminium Upper Door Piece", "gd_building_h1_p07" , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Aluminium Wall 01", "gd_building_h1_p08" , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Aluminium Wall 02", "gd_building_h1_p09" , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Aluminium Wall 03", "gd_building_h1_p10" , "Buildings", "Building Supplies", false)
AddToGDSpawnMenu("Foundation", "gd_building_h1_p11" , "Buildings", "Building Supplies", false)

--[[ 
Building 02
--]]

AddToGDSpawnMenu("Storm Shelter", "gd_equip_shelter" , "Buildings", "Storm Shelter", false)
AddToGDSpawnMenu('6.7" Solid Steel Door', "gd_equip_shelter_d" , "Buildings", "Storm Shelter", false)


--[[
Equipment 
--]]

AddToGDSpawnMenu("Dorothy Probe", "gd_equip_dorothy_probe" , "Equipment", "Dorothy", false)
AddToGDSpawnMenu("Dorothy IV", "gd_equip_dorothy" , "Equipment", "Dorothy", false)
AddToGDSpawnMenu("Tornado Siren", "gd_equip_siren" , "Equipment", "Standard", false)

--[[
WEATHER TIER 1
--]]

AddToGDSpawnMenu("Sand Storm", "gd_w1_sandstorm" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Dust Storm", "gd_w1_duststorm" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Heavy Fog", "gd_w1_heavyfog" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Heavy Rain", "gd_w1_heavyrain" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Light Snow", "gd_w1_snow" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Sunny", "gd_w1_sunny" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Iridescent Cloudy", "gd_w1_cldy_irid" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Sleet", "gd_w1_sleet" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Aurora Borealis", "gd_w1_aurora" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Light Rain", "gd_w1_lightrain" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Night", "gd_w1_night" , "Weather", "Tier 1", false)
AddToGDSpawnMenu("Tropical Depression", "gd_w1_tropicaldepression", "Weather", "Tier 1", false)
AddToGDSpawnMenu("Tropical Storm", "gd_w1_tropicalstorm", "Weather", "Tier 1", false)
AddToGDSpawnMenu("Category 1 Hurricane", "gd_w1_catonehurricane", "Weather", "Tier 1", false)
AddToGDSpawnMenu("Light Wind", "gd_w1_lightbreeze", "Weather", "Tier 1", false)
AddToGDSpawnMenu("Beaufort 0 Wind (Calm)", "gd_w1_beaufort0", "Weather", "Tier 1", false)
AddToGDSpawnMenu("Beaufort 1 Wind (Light Air)", "gd_w1_beaufort1", "Weather", "Tier 1", false)

--[[
WEATHER TIER 2
--]]

AddToGDSpawnMenu("Acid Rain", "gd_w2_acidrain" , "Weather", "Tier 2", false)
AddToGDSpawnMenu("Heat Wave", "gd_w2_heatwave" , "Weather", "Tier 2", false)
AddToGDSpawnMenu("Blizzard", "gd_w2_blizzard" , "Weather", "Tier 2", false)
AddToGDSpawnMenu("Heat Burst", "gd_w2_heatburst" , "Weather", "Tier 2", false)
AddToGDSpawnMenu("Hail Storm", "gd_w2_hailstorm" , "Weather", "Tier 2", false)
AddToGDSpawnMenu("Ash Storm", "gd_w2_volcano_ash" , "Weather", "Tier 2", false)
AddToGDSpawnMenu("Moderate Wind", "gd_w2_modbreeze" , "Weather", "Tier 2", false)
AddToGDSpawnMenu("Cold Snap", "gd_w2_coldwave" , "Weather", "Tier 2", false)
AddToGDSpawnMenu("Heavy Snow", "gd_w2_heavysnow" , "Weather", "Tier 2", false)
AddToGDSpawnMenu("Category 2 Hurricane", "gd_w2_cattwohurricane", "Weather", "Tier 2", false)
AddToGDSpawnMenu("Beaufort 2 Wind (Light Breeze)", "gd_w2_beaufort2", "Weather", "Tier 2", false)
AddToGDSpawnMenu("Beaufort 3 Wind (Gentle Breeze)", "gd_w2_beaufort3", "Weather", "Tier 2", false)

--[[
WEATHER TIER 3--gd_w3_major_hailstorm
--]]

AddToGDSpawnMenu("Strong Wind" , "gd_w3_strongbreeze" , "Weather", "Tier 3", false)
AddToGDSpawnMenu("Major Hailstorm" , "gd_w3_major_hailstorm" , "Weather", "Tier 3", false)
AddToGDSpawnMenu("Ice Storm", "gd_w3_icestorm" , "Weather", "Tier 3", false)
AddToGDSpawnMenu("Extreme Heavy Rain", "gd_w3_extremerain" , "Weather", "Tier 3", false)
AddToGDSpawnMenu("Category 3 Hurricane", "gd_w3_catthreehurricane", "Weather", "Tier 3", false)
AddToGDSpawnMenu("Beaufort 4 Wind (Moderate Breeze)", "gd_w3_beaufort4", "Weather", "Tier 3", false)
AddToGDSpawnMenu("Beaufort 5 Wind (Fresh Breeze)", "gd_w3_beaufort5", "Weather", "Tier 3", false)
AddToGDSpawnMenu("Martian Dust Storm", "gd_w3_martianduststorm", "Weather", "Tier 3", false)

--[[
WEATHER TIER 4
--]]

AddToGDSpawnMenu("Intense Wind" , "gd_w4_intensebreeze" , "Weather", "Tier 4", false)
AddToGDSpawnMenu("Category 4 Hurricane", "gd_w4_catfourhurricane", "Weather", "Tier 4", false)
AddToGDSpawnMenu("Beaufort 6 Wind (Strong Breeze)", "gd_w4_beaufort6", "Weather", "Tier 4", false)
AddToGDSpawnMenu("Beaufort 7 Wind (High Wind, Moderate Gale, Near Gale)", "gd_w4_beaufort7", "Weather", "Tier 4", false)

--[[
WEATHER TIER 5
--]]

AddToGDSpawnMenu("Very Intense Wind" , "gd_w5_veryintensebreeze" , "Weather", "Tier 5", false)
AddToGDSpawnMenu("Category 5 Hurricane", "gd_w5_catfivehurricane", "Weather", "Tier 5", false)
AddToGDSpawnMenu("Beaufort 8 Wind (Gale, Fresh Gale)", "gd_w5_beaufort8", "Weather", "Tier 5", false)
AddToGDSpawnMenu("Beaufort 9 Wind (Strong Gale, Severe Gale)", "gd_w5_beaufort9", "Weather", "Tier 5", false)

--[[
WEATHER TIER 6
--]]
--
AddToGDSpawnMenu("Downburst" , "gd_w6_downburst"    , "Weather", "Tier 6", false)
AddToGDSpawnMenu("Superheated Firestorm" , "gd_w6_solarray"    , "Weather", "Tier 6", false)
AddToGDSpawnMenu("Category Six Hurricane" , "gd_w6_catsixhurricane"    , "Weather", "Tier 6", false)
AddToGDSpawnMenu("Like Really Cold" , "gd_w6_freezeray"    , "Weather", "Tier 6", false)
AddToGDSpawnMenu("Extreme Wind" , "gd_w6_extremebreeze"    , "Weather", "Tier 6", false)
AddToGDSpawnMenu("Beaufort 10 Wind (Storm, Whole Gale)", "gd_w6_beaufort10", "Weather", "Tier 6", false)
AddToGDSpawnMenu("Beaufort 11 Wind (Violent Storm)", "gd_w6_beaufort11", "Weather", "Tier 6", false)
AddToGDSpawnMenu("Beaufort 12 Wind (Hurricane Force)", "gd_w6_beaufort12", "Weather", "Tier 6", false)
AddToGDSpawnMenu("Martian Snow Storm", "gd_w6_martiansnow", "Weather", "Tier 6", false)

--[[
DISASTERS 
TIER ONE
--]]

AddToGDSpawnMenu("Dust Devil"     , "gd_d1_dustdevil"       , "Disasters", "Tier 1", false)
AddToGDSpawnMenu("Patch of Ice"   , "gd_d1_ice"				, "Disasters", "Tier 1", false)
AddToGDSpawnMenu("≈1.0 - 1.9 Earthquake" , "gd_d1_rs1eq"	, "Disasters", "Tier 1", false)
AddToGDSpawnMenu("Small Whirlpool" , "gd_d1_whirlpool"		, "Disasters", "Tier 1", false)
AddToGDSpawnMenu("Tar Pit" , "gd_d1_tarpit"		, "Disasters", "Tier 1", false)
AddToGDSpawnMenu("Quicksand" , "gd_d1_quicksand"		, "Disasters", "Tier 1", false)
AddToGDSpawnMenu("Supercooled Ice" , "gd_d1_supercooledice"		, "Disasters", "Tier 1", false)
AddToGDSpawnMenu("Rainbow" , "gd_d1_rainbow"		, "Disasters", "Tier 1", false)

--[[
DISASTERS 
TIER TWO
--]]

AddToGDSpawnMenu("Medium Whirlpool" , "gd_d2_whirlpool"			, "Disasters", "Tier 2", false)
AddToGDSpawnMenu("Gustnado"         , "gd_d2_gustnado"			, "Disasters", "Tier 2", false)
AddToGDSpawnMenu("≈2.0 - 2.9 Earthquake", "gd_d2_rs2eq"       	, "Disasters", "Tier 2", false)
AddToGDSpawnMenu("Locust Apocalypse"        , "gd_d2_locust_apoc" , "Disasters", "Tier 2", false)
AddToGDSpawnMenu("Geyser"        , "gd_d2_geyser" , "Disasters", "Tier 2", false)
AddToGDSpawnMenu("Hail"        , "gd_d2_hail" , "Disasters", "Tier 2", false)
AddToGDSpawnMenu("Bolt of Lightning"        , "gd_d2_lightning" , "Disasters", "Tier 2", false)
AddToGDSpawnMenu("Mini Volcano"        , "gd_d2_minivolcano" , "Disasters", "Tier 2", false)
AddToGDSpawnMenu("Black Ice Over"        , "gd_d2_blackiceover" , "Disasters", "Tier 2", false)
AddToGDSpawnMenu("Flash Flood"        , "gd_d2_flashflood"              , "Disasters", "Tier 2", false)
AddToGDSpawnMenu("Micro Firenado"        , "gd_d2_mfirenado"              , "Disasters", "Tier 2", false)

--[[
DISASTERS 
TIER THREE
--]]

AddToGDSpawnMenu("Ball Lightning"        , "gd_d3_ball_lightning"              , "Disasters", "Tier 3", false)
AddToGDSpawnMenu("Lightning Storm"        , "gd_d3_lightningstorm"              , "Disasters", "Tier 3", false)
AddToGDSpawnMenu("Big Whirlpool"      , "gd_d3_whirlpool"        , "Disasters", "Tier 3", false)
AddToGDSpawnMenu("≈3.0 - 3.9 Earthquake" , "gd_d3_rs3eq"         , "Disasters", "Tier 3", false)
AddToGDSpawnMenu("EF0 Tornado"        , "gd_d3_ef0"              , "Disasters", "Tier 3", false)

--[[
DISASTERS 
TIER FOUR
--]]

AddToGDSpawnMenu("≈4.0 - 4.9 Earthquake" , "gd_d4_rs4eq"         , "Disasters", "Tier 4", false)
AddToGDSpawnMenu("EF1 Tornado"        , "gd_d4_ef1"              , "Disasters", "Tier 4", false)
AddToGDSpawnMenu("Megacryometeor"        , "gd_d4_mcryometeor"              , "Disasters", "Tier 4", false)
AddToGDSpawnMenu("♫ The Man In Gauze ♫"        , "gd_d4_themaningauze"              , "Disasters", "Tier 4", false)
AddToGDSpawnMenu("Young Volcano"        , "gd_d4_volcano"              , "Disasters", "Tier 4", false)


--[[
DISASTERS 
TIER FIVE
--]]

AddToGDSpawnMenu("EF2 Tornado"           , "gd_d5_ef2"           , "Disasters", "Tier 5", false)
AddToGDSpawnMenu("≈5.0 - 5.9 Earthquake" , "gd_d5_rs5eq"         , "Disasters", "Tier 5", false)
AddToGDSpawnMenu("Mini Firenado" , "gd_d5_mfirenado"         , "Disasters", "Tier 5", false)

--[[
DISASTERS 
TIER SIX
--]]

AddToGDSpawnMenu("EF3 Tornado"           , "gd_d6_ef3"           , "Disasters", "Tier 6", false)
--AddToGDSpawnMenu("EF3 Tornado (Smart)"   , "gd_d6_ef3_pathed"    , "Disasters", "Tier 6", false)
AddToGDSpawnMenu("≈6.0 - 6.9 Earthquake" , "gd_d6_rs6eq"         , "Disasters", "Tier 6", false)
AddToGDSpawnMenu("Cryo Shower" , "gd_d6_cryoshower"         , "Disasters", "Tier 6", false)
AddToGDSpawnMenu("Mature Volcano" , "gd_d5_maturevolcano"         , "Disasters", "Tier 6", false)



--[[
DISASTERS 
TIER SEVEN
--]]

AddToGDSpawnMenu("EF4 Tornado"           , "gd_d7_ef4"           , "Disasters", "Tier 7", false)
AddToGDSpawnMenu("≈7.0 - 7.9 Earthquake" , "gd_d7_rs7eq"         , "Disasters", "Tier 7", false)
AddToGDSpawnMenu("Tsunami" , "gd_d7_tsunami"         , "Disasters", "Tier 7", false)

--[[
DISASTERS 
TIER EIGHT
--]]

AddToGDSpawnMenu("EF5 Tornado"           , "gd_d8_ef5"           , "Disasters", "Tier 8", false)
--AddToGDSpawnMenu("EF5 Tornado (Smart)"   , "gd_d8_ef5_pathed"    , "Disasters", "Tier 8", false)
AddToGDSpawnMenu("≈8.0 - 8.9 Earthquake" , "gd_d8_rs8eq"         , "Disasters", "Tier 8", false)

--[[
DISASTERS 
TIER NINE
--]]

AddToGDSpawnMenu("≈9.0 - 9.9 Earthquake" , "gd_d9_rs9eq"         , "Disasters", "Tier 9", false)

--[[
DISASTERS 
TIER TEN
--]]

AddToGDSpawnMenu("10.0 Earthquake" , "gd_d10_rs10eq"         , "Disasters", "Tier ELE", false)
AddToGDSpawnMenu("Neutron Star"  , "gd_d10_neutron_star" , "Disasters", "Tier ELE", false)
AddToGDSpawnMenu("Martian Tornado"  , "gd_d10_martiantornado" , "Disasters", "Tier ELE", false)
AddToGDSpawnMenu("Firenado"  , "gd_d10_mfirenado" , "Disasters", "Tier ELE", false)
AddToGDSpawnMenu("Mega Tsunami"  , "gd_d10_megatsunami" , "Disasters", "Tier ELE", false)

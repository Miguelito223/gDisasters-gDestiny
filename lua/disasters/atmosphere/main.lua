...
function AtmosphereFadeControl()
	--GLOBAL_SYSTEM_TARGET =  {["Atmosphere"] 	= {["Wind"]        = {["Speed"]=0,"Direction"]=Vector(1,0,0)}, ["Pressure"]    = 100000, ["Temperature"] = 23, ["Humidity"]    = 25, ["BRadiation"]  = 0.1}}
...
function Wind()
	...
	for k, v in pairs(player.GetAll()) do
		...
		local windvel_new         = windvel + frictional_velocity
...
if (CLIENT) then
	...
	function gDisasters_WindControl()
		...
		if LocalPlayer().Sounds["Wind_Heavy"] == nil then
			...
			LocalPlayer().Sounds["Wind_Light"]         = createLoopedSound(LocalPlayer(), "disasters/nature/wind_weak.wav")
			LocalPlayer().Sounds["Wind_Moderate"]      = createLoopedSound(LocalPlayer(), "disasters/nature/wind_moderate.wav")
			LocalPlayer().Sounds["Wind_Heavy"]         = createLoopedSound(LocalPlayer(), "disasters/nature/wind_heavy.wav")

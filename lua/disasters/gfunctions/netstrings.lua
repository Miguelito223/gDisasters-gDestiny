if (SERVER) then
	...
	util.AddNetworkString( "gd_darkness_wtfrm" )
	util.AddNetworkString( "gd_darkness_stun" )
...
net.Receive("gd_darkness_stun", function()
	
	local t       = net.ReadFloat() or 3
	local fadein  = net.ReadFloat() or 0.1
	local fadeout = net.ReadFloat() or 0.9
	
	gfx_SilentHillStun(color, t, math.Clamp(fadein, 0, 1), math.Clamp(fadeout, 0, 1))
 
end)



net.Receive("gd_darkness_wtfrm", function()
	
	local enable_darkness = net.ReadBool() or false
	gfx_SilentHillDarkness(enable_darkness)
